...
public class 浏览框
    ...
    private void init() {
        this.mProgressBar = new ProgressBar(窗口环境, null, android.R.attr.progressBarStyleHorizontal);
        this.mProgressBar.setLayoutParams(new AbsoluteLayout.LayoutParams(-1, this.dip2px(2), 0, 0));
        this.getView().addView(this.mProgressBar);
        this.getView().setWebChromeClient(new MyWebChromeClient());
        this.getView().setWebViewClient(new MyWebViewClient());
        this.getView().setDownloadListener(new MyDownloadListener());
    ...
    public void 添加js接口(String string) {
        this.getView().addJavascriptInterface(new JsInterface(), string);
    }

    public long 下载(String string, String string2) {
        DownloadManager downloadManager = (DownloadManager) this.窗口环境.getSystemService(Context.DOWNLOAD_SERVICE);
    ...
    public void 回调(int n, int n2, 意图 意图2) {
        if (n == 5173) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                message_upload2.onReceiveValue(WebChromeClient.FileChooserParams.parseResult(n2, 意图2));
            }
    ...
    public interface js交互事件 {
        void js交互事件(String var1);
    }

    public interface 拦截到下载 {
        void 拦截到下载(String var1, String var2, String var3, long var4);
    }

    public interface 拦截到请求 {
        void 拦截到请求(String var1);
    }

    public interface 拦截到Uri {
        void 拦截到Uri(String var1);
    }

    public interface 接收到标题 {
        void 接收到标题(String var1);
    }

    public interface 进度值改变 {
        void 进度值改变(int var1);
    }

    public interface 网页加载完成 {
        void 网页加载完成(String var1);
    }

    public interface 网页开始加载 {
        void 网页开始加载(String var1);
    }
    ...
    private class MyWebChromeClient
        ...
        public void onProgressChanged(WebView webView, int n) {
            ...
            if (n == 100) {
                浏览框.this.mProgressBar.setVisibility(View.GONE);
            } else {
                if (浏览框.this.mProgressBar.getVisibility() == View.GONE) {
                    浏览框.this.mProgressBar.setVisibility(View.VISIBLE);
                }
        ...
        public void onHideCustomView() {
            ...
            浏览框.this.closeVideo();
            浏览框.this.getView().setVisibility(View.VISIBLE);
        }
        ...
        public void openFileChooser(ValueCallback<Uri> valueCallback, String string, String string2) {
            if (浏览框.this.窗口环境 instanceof Activity) {
                ...
                intent.setType("*/*");
                activity.startActivityForResult(Intent.createChooser(intent, "文件选择"), 5173);
            }
        }
