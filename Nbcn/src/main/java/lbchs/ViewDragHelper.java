...
public class ViewDragHelper {
    ...
    private int computeAxisDuration(int n, int n2, int n3) {
        ...
        int n6 = n5 / 2;
        float f = Math.min(1.0f, (float) Math.abs(n) / (float) n5);
        float f2 = (float) n6 + (float) n6 * this.distanceInfluenceForSnapDuration(f);
        if ((n2 = Math.abs(n2)) > 0) {
            n4 = 4 * Math.round(1000.0f * Math.abs(f2 / (float) n2));
        } else {
            float f3 = (float) Math.abs(n) / (float) n3;
            n4 = (int) ((f3 + 1.0f) * 256.0f);
        }
        return Math.min(n4, 600);
    }
    ...
    protected boolean canScroll(View view, int n, int n2, int n3, int n4) {
        if (view instanceof ViewGroup) {
            ...
            for (int i = n7 - 1; i >= 0; --i) {
                ...
                if (n3 + n5 < view2.getLeft() || n3 + n5 >= view2.getRight() || n4 + n6 < view2.getTop() || n4 + n6 >= view2.getBottom() || !this.canScroll(view2, n, n2, n3 + n5 - view2.getLeft(), n4 + n6 - view2.getTop()))
                    continue;
                return true;
            }
        }
        return (view.canScrollHorizontally(-n) || view.canScrollVertically(-n2));
    }
        ...
        switch (n) {
            ...
            case 2: {
                ...
                for (int i = 0; i < n7; ++i) {
                    ...
                    boolean bl2 = bl = this.checkTouchSlop(view, f5, f6);
                    if (bl) {
                        int n9 = view.getLeft();
                        int n10 = n9 + (int) f5;
            ...
            case 6: {
                ...
                if (this.mDragState == 1 && n12 == this.mActivePointerId) {
                    ...
                    for (int i = 0; i < n14; ++i) {
                        ...
                        if (n15 == this.mActivePointerId || this.findTopChildUnder((int) motionEvent.getX(i), (int) motionEvent.getY(i)) != this.mCapturedView || !this.tryCaptureViewForDrag(this.mCapturedView, n15))
                            continue;
    ...
    private void reportNewEdgeDrags(float f, float f2, int n) {
        ...
        if (n2 != 0) {
            this.mEdgeDragsInProgress[n] = this.mEdgeDragsInProgress[n] | n2;
    ...
    private boolean checkNewEdgeDrag(float f, float f2, int n, int n2) {
        ...
        if (f3 < f4 * 0.5f && this.mCallback.onEdgeLock(n2)) {
            this.mEdgeDragsLocked[n] = this.mEdgeDragsLocked[n] | n2;
            return false;
        }
