...
public class 圆形图片框
        extends 可视化组件 {
    private static SparseArray<ScaleType> intToType;

    static {
        intToType = new SparseArray<ScaleType>();
    ...
    public void 图片(String string) {
        if (string.startsWith("http")) {
            ...
            this.getView().setImageBitmap(BitmapFactory.decodeFile((String) string));
        } else {
            try {
    ...
    public void 边框颜色(String string) {
        this.边框颜色(Color.parseColor((String) string));
    }
    ...
    public void 载入字节图片(byte[] byArray) {
        this.getView().setImageBitmap(BitmapFactory.decodeByteArray((byte[]) byArray, (int) 0, (int) byArray.length));
    }
