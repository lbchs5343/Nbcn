...
class RoundRectDrawableWithShadow
        extends Drawable {
    static final double COS_45 = Math.cos((double) Math.toRadians((double) 45.0));
    ...
    public boolean getPadding(Rect rect) {
        int n = (int) Math.ceil((double) RoundRectDrawableWithShadow.calculateVerticalPadding(this.mRawMaxShadowSize, this.mCornerRadius, this.mAddPaddingForCorners));
        int n2 = (int) Math.ceil((double) RoundRectDrawableWithShadow.calculateHorizontalPadding(this.mRawMaxShadowSize, this.mCornerRadius, this.mAddPaddingForCorners));
        rect.set(n2, n, n2, n);
        return true;
    }
    ...
    private void buildShadowCorners() {
        ...
        this.mCornerShadowPaint.setShader((Shader) new RadialGradient(0.0f, 0.0f, this.mCornerRadius + this.mShadowSize, new int[]{this.mShadowStartColor, this.mShadowStartColor, this.mShadowEndColor}, new float[]{0.0f, f, 1.0f}, Shader.TileMode.CLAMP));
    ...
    float getMinWidth() {
        float f = 2.0f * Math.max((float) this.mRawMaxShadowSize, (float) (this.mCornerRadius + (float) this.mInsetShadow + this.mRawMaxShadowSize / 2.0f));
        return f + (this.mRawMaxShadowSize + (float) this.mInsetShadow) * 2.0f;
    }

    float getMinHeight() {
        float f = 2.0f * Math.max((float) this.mRawMaxShadowSize, (float) (this.mCornerRadius + (float) this.mInsetShadow + this.mRawMaxShadowSize * 1.5f / 2.0f));
