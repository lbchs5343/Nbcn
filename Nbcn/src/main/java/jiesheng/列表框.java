...
public class 列表框
    ...
    public 列表框(Context context) {
        ...
        this.adp = new ArrayAdapter(context, 17367043, this.items);
        ...
        listView.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScrollStateChanged(AbsListView absListView, int n) {
                switch (n) {
                    case 0: {
                        列表框.this.isStop = true;
                        break;
                    }
                    default: {
                        列表框.this.isStop = false;
                    }
                }
    ...
    public void 分割线颜色(int n) {
        this.getView().setDivider((Drawable) new ColorDrawable(n));
    }

    public void 分割线颜色(String string) {
        this.getView().setDivider((Drawable) new ColorDrawable(Color.parseColor((String) string)));
    }

    public int 分割线颜色() {
        return ((ColorDrawable) this.getView().getDivider()).getColor();
    }
    ...
    public void 置适配器(适配器 适配器2) {
        this.adapter = 适配器2;
        this.getView().setAdapter((ListAdapter) 适配器2);
    }

    public void 置适配器(高级适配器 高级适配器2) {
        this.adapter = 高级适配器2;
        this.getView().setAdapter((ListAdapter) 高级适配器2);
    }

    public void 置适配器(基础适配器 基础适配器2) {
        this.adapter = 基础适配器2;
        this.getView().setAdapter((ListAdapter) 基础适配器2);
    }
    ...
    public void 项目(String string) {
        if (this.adapter == null && string.contains((CharSequence) "{") && string.contains((CharSequence) "}")) {
    ...
    public static interface 滚动到底部 {
        public void 滚动到底部();
    }

    public static interface 滚动到顶部 {
        public void 滚动到顶部();
    }

    public static interface 正在下滚 {
        public void 正在下滚();
    }

    public static interface 正在上滚 {
        public void 正在上滚();
    }

    public static interface 滚动状态改变 {
        public void 滚动状态改变(int var1);
    }

    public static interface 项目被长按 {
        public void 项目被长按(int var1);
    }
