...
public class 高级适配器
    ...
    public 高级适配器(Object[] objectArray) {
    ...
    public View getView(int n, View view, ViewGroup viewGroup) {
        组件容器 组件容器2 = this.$event_internal_加载项目 != null ? this.$event_internal_加载项目.加载项目(n, view == null ? null : (组件容器) view.getTag()) : null;
        return ((可视化组件) 组件容器2.取根组件()).getView();
    }
    ...
    public <通用型> 通用型 取项目(int n) {
        return (通用型) this.getItemTraversal(n);
    }
    ...
    public static interface 加载项目 {
        public 组件容器 加载项目(int var1, 组件容器 var2);
    }
}
